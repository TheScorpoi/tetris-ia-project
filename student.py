...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        ...
        if action in self.actions(city):
            for ligacao in self.connections:
                if ( ligacao[0] == C1 and ligacao[1] == C2 ) or ( ligacao[0] == C2 and ligacao[1] == C1 ):
                    return ligacao[2]
        pass
    def heuristic(self, city, goal_city):
        coordenadas_city = self.coordinates.get(city)
        coordenadas_goal_city = self.coordinates.get(goal_city)
        return math.sqrt((coordenadas_city[0] - coordenadas_goal_city[0]) ** 2 + (coordenadas_city[1] - coordenadas_goal_city[1]) ** 2)
